...
if (msg.text === 'Групписо призывус! 📜') {
  ...
  // =================================ПОДДЕРЖКА===================================
 // --- Если пользователь ждёт поддержки, пересылаем его сообщение и даём подтверждение ---
if (waitingSupport[msg.from.id]) {
  waitingSupport[msg.from.id] = false; // Сбросить ожидание
  ...
  bot.sendMessage(msg.chat.id, "Твой вопрос уже летит к магистру 🕊 Ожидай ответа в магическом чате ✨");
  return;
}
...
bot.onText(/\/support/, (msg) => {
  waitingSupport[msg.from.id] = true; // Пользователь ждет поддержки
  bot.sendMessage(msg.chat.id,
    "Если у тебя есть вопрос, пожелание или что-то не работает — просто опиши проблему в следующем сообщении. Магистр прочитает и обязательно ответит! ✉️"
  );
});
...
bot.onText(/\/privacy/, (msg) => {
  const privacyText = `
🛡 <b>Магическая безопасность и приватность</b>

🔒 Мы не храним ваши пароли и не передаем токены третьим лицам. Все ключи используются только для волшебной доставки новостей из ваших VK-групп и защищены чарами серверной безопасности.

• <b>Данные нужны только для магии:</b> ваш Telegram ID и временный VK-токен используются лишь чтобы пересылать новости.
• <b>Ваши пароли никому не видны</b> — даже магистру Альтаиру.
• <b>В любой момент можно отключить магию</b> — просто выйдите из бота или удалите его.

Полную политику конфиденциальности можно прочитать здесь:
<a href="https://api.fokusnikaltair.xyz/privacy.html">https://api.fokusnikaltair.xyz/privacy.html</a>
`;

  bot.sendMessage(msg.chat.id, privacyText, { parse_mode: "HTML", disable_web_page_preview: false });
});
